...
void myimgui_init ( SDL_Window * window , SDL_GLContext gl_context , const char * glsl_version ) {
	...
	io . Fonts -> AddFontFromFileTTF ( "fonts/荆南麦圆体.ttf" , 16.0f , & config ,
		io . Fonts -> GetGlyphRangesChineseFull ( ) ) ; ///usr/share/fonts/freefontsCJK/荆南麦圆体.ttf
...
void myimgui_draw ( ) {
	if ( false ) { // 1. Show the big demo window (Most of the sample code is in ImGui::ShowDemoWindow()! You can browse its code to learn more about Dear ImGui!).
		if ( show_demo_window )
