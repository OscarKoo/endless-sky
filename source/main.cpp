...
int main ( int argc , char * argv [ ] ) {
	tfg = tfg_init ( 0 ) ; // init the jvm and prepair
	tfg_include ( tfg , "endsky.tfg" ) ; // load the script and call init()
	tfg_call ( tfg , "mylog" , 1 , "start tfg..." ) ; // call the method
	tfg_call ( tfg , "mylog" , 1 , "let : \"me\" see ..." ) ; // call the method
	tfg_call ( tfg , "mylog" , 1 , "no error?." ) ;
	tfg_call ( tfg , "mylog" , 1 , "really?" ) ;
	tfg_flushLog ( ) ;
...
Conversation LoadConversation ( ) {
	...
	map < string , string > subs = { { "<bunks>" , "[N]" } , { "<cargo>" , "[N tons of Commodity]" } , { "<commodity>" , "[Commodity]" } , { "<date>" , "[Day Mon Year]" } , { "<day>" , "[The Nth of Month]" } , { "<destination>" , "[Planet in the Star system]" } , { "<fare>" , "[N passengers]" } , { "<first>" , "[First]" } , { "<last>" , "[Last]" } , { "<origin>" , "[Origin Planet]" } , { "<passengers>" , "[your passengers]" } , { "<planet>" , "[Planet]" } , { "<ship>" , "[Ship]" } , { "<system>" , "[Star]" } , { "<tons>" , "[N tons]" }
	} ;
